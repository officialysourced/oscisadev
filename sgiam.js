...
function updateActivity(userData) {
    ...
    // Always call updateSpotifyDisplay to ensure it hides if no Spotify activity
    const spotifyActivity = userData.activities ? userData.activities.find(activity =>
        activity.name === 'Spotify' && activity.type === 2
    ) : null;
    
    if (spotifyActivity) {
        updateSpotifyDisplay(spotifyActivity);
        activityElement.textContent = `Listening to Spotify`;
    } else {
        updateSpotifyDisplay(null); // Hide Spotify section
